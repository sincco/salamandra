...
class ImagesController extends Sincco\Sfphp\Abstracts\Controller 
{
	...
	private function createFileFromChunks($temp_dir, $fileName, $chunkSize, $totalSize,$total_files, $subDir) {
		...
		if ($total_files_on_server_size >= $totalSize) {
			if (! is_dir(PATH_PROJECTS_FILES . '/' . $subDir)) {
				mkdir(PATH_PROJECTS_FILES . '/' . $subDir, 0775, true);
			}
			// Directorio final
			if (($fp = fopen(PATH_PROJECTS_FILES . '/' . $subDir . '/' . $fileName, 'w')) !== false) {
				for ($i=1; $i<=$total_files; $i++) {
					fwrite($fp, file_get_contents($temp_dir.'/'.$fileName.'.part'.$i));
					var_dump('escribiendo parte '.$i);
				}
				fclose($fp);
				chmod(PATH_PROJECTS_FILES . '/' . $subDir . '/' . $fileName, 0777);
			} else {
				var_dump('no se pudo escribir el archivo final',PATH_PROJECTS_FILES . '/' . $subDir . '/' . $fileName);
				return false;
			}
