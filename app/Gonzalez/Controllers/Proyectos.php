...
class ProyectosController extends Sincco\Sfphp\Abstracts\Controller {
	...
	public function formato() {
		...
		$usuariosExtra = $this->getModel('Salamandra');
		...
		$view->vendedor = $user['nombre'];
		$view->telefono1 = $user['telefono1'];
	...
	public function costoLabor() {
		$model = $this->getModel('Salamandra');
		$data = $this->getParams('data');
		$data = $data[0]['data'];
		$data['idProyecto'] = $this->getParams('data')[0]['idProyecto'];
		$model->init();
		$id = $model->gzlzProyectoCstoLabor()->insert($data);
