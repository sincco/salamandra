...
class UnidadesModel extends Sincco\Sfphp\Abstracts\Model {
	...
	public function getAll() {
		$query = 'SELECT uni.idUnidad, uni.noEco, uni.idSocio, soc.nombre, uni.estatus FROM unidades uni INNER JOIN socios soc USING(idSocio);';
		return $this->connector->query( $query );
	}

	public function getById( $data ) {
		return $this->connector->query('SELECT  * FROM unidades WHERE idSocio = :idSocio
			ORDER BY nombre');
	}
